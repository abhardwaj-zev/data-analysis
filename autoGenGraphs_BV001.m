function autoGenGraphs_BV001(autoGenChoiceList,varTable,date,title_plot,dat1,dat2)
    ...
    if(motor)
        yi = [16,18,91,92,30]; yi_r=[9];                                      % Motor temps check 
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);

        yi = [16,18,91,92,30]; yi_r=[108];                                      % Motor temps check 
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
        
        yi = [17];                      yi_r=[15];                        % Motor oil pressure check 
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);

        yi = [17];                      yi_r=[98];                        % Motor oil pressure check 
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
    end
    if(inv)
        yi = [19,20,21,89,90,30];      yi_r=[9];                              % Inv check
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
              
        yi = [20,55,56,30];         yi_r=[9];                              % Inv Check
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
        
        yi = [20,55,56,30];         yi_r=[31];                              % Inv Check
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);        
    end  
    if(dcdc)
        yi=[22,27,91,30];  yi_r=[9];
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);

        yi=[22,27,91,30];  yi_r=[108];
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
    end
    ...
    if(battery)
        ...
        yi = [1000,1001,1002,1005,1006,1007,26,88,30]; yi_r=[];                          % Battery Module High Level Temps
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
...
%         plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
        ...
        yi = [33:38,26,88,1000,1005,30];     yi_r=[];                            % Modules 1-3
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
        
        yi = [39:44,26,88,1000,1005,30];   yi_r=[];                              % Modules 4-5
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
        
        yi = [45:50,26,88,1000,1005,30];   yi_r=[];                              % Modules 7-9
        plotVar(xi,yi,dat1,dat2,varTable,yi_r,date,title_plot);
